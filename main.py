...
class App():
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            'Report Generator V1.0.0 ©2020 VseA (Changshu) Co., Ltd')
        ...
        Label(Tops, text='Excel Report Generator', font=(
            'Arial', 20, 'bold'), justify=CENTER).grid(padx=2)
        status = Label(Tops, text='waiting for your select...',
                       bg="#c0ded9", font=('Arial', 12, 'bold'), justify=CENTER)
        status.grid(pady=25)

        MembersPath = LabelFrame(Mainframe, bd=4, width=466, height=200, font=(
            'Arial', 12), text='', relief=RIDGE)
        MembersPath.pack(side=TOP)

        ButtonFrame = LabelFrame(Mainframe, bd=2, width=466, height=20, font=(
            'Arial', 12), text='', relief=RIDGE)
        ButtonFrame.pack(side=BOTTOM)

        margin = LabelFrame(Mainframe, bd=0, width=46, height=20, font=(
            'Arial', 12), text='', relief=RIDGE)
                ...
                if len(HipotPath) > 0:
                    try:
                        if HipotData_To_Report(HipotPath) == None:
                            ...
                            return tkinter.messagebox.showinfo("Warnning", "数据写入异常!!")
                    except FileNotFoundError:
                        status['text'] = 'waiting for your select...'
                        status['bg'] = '#c0ded9'
                        tkinter.messagebox.showinfo(
                            "Warnning", "Please select correctly path...")
                        return False
                else:
                    tkinter.messagebox.showinfo(
                        "Warnning", "Please select the Hipot path!")
                    return False
        ...
        def EOL_Builder():
            with LOCK:
                ...
                if len(EOLPath) > 0:
                    ...
                    tkinter.messagebox.showinfo(
                        "Warnning", "Please select the EOL path!")
                    return False
        ...
        def GRR_Builder():
            with LOCK:
                ...
                if len(GRRPath) > 0:
                    ...
                    tkinter.messagebox.showinfo(
                        "Warnning", "Please select the GRR path!")
                    return False
        ...
        self.btnEOLPath = Button(MembersPath, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="EOL Path", command=OpenEOLPath, bg="#c0ded9")
        self.btnEOLPath.grid(row=0, column=0, pady=2)  #
        self.txtNumberOne = Entry(MembersPath, font=(
            'Arial', 13, 'bold'), bd=7, text=Entry_EOL, width=34)
        self.txtNumberOne.grid(row=0, column=1)

        self.btnHipotPaht = Button(MembersPath, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="Hipot Path", command=OpenHipotPath, bg="#c0ded9")
        self.btnHipotPaht.grid(row=1, column=0, pady=2)
        self.txtNumberTwo = Entry(MembersPath, font=(
            'Arial', 13, 'bold'), bd=7, text=Entry_Hipot, width=34)
        self.txtNumberTwo.grid(row=1, column=1)

        self.btnGRR = Button(MembersPath, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="GRR Path", command=OpenGRRPath, bg="#c0ded9")
        self.btnGRR.grid(row=2, column=0, pady=2)
        self.txtNumerThree = Entry(MembersPath, font=(
            'Arial', 13, 'bold'), bd=7, text=Entry_GRR, width=34)
        self.txtNumerThree.grid(row=2, column=1)

        # =======================================================Button===========================================================================================================
        self.btnRest = Button(ButtonFrame, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="Reset", command=Reset, bg="#c0ded9")
        self.btnRest.grid(row=10, column=3, padx=12)
        self.btnHipotStart = Button(ButtonFrame, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="Hipot Builder", command=HipotpathThreading, bg="#c0ded9")
        self.btnHipotStart.grid(row=10, column=2, padx=12)
        self.btnGRRStart = Button(ButtonFrame, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="GRR Builder", command=GRRpathThreading, bg="#c0ded9")
        self.btnGRRStart.grid(row=10, column=0, padx=12)
        self.btnEOLStart = Button(ButtonFrame, padx=18, bd=7, font=(
            'Arial', 10, 'bold'), width=7, text="EOL Builder", command=EOLpathThreading, bg="#c0ded9")
